...
const InteractiveNeuralVortex: React.FC<InteractiveNeuralVortexProps> = ({ className }) => {
    ...
    `;
        ...
        // Green/Neon themed fragment shader
        const fsSource = `
      precision mediump float;
      varying vec2 vUv;
      ...
      void main() {
        ...
        // Neon green color scheme (#00FF9C)
        color = vec3(0.0, 0.6, 0.35);
        color = mix(color, vec3(0.0, 1.0, 0.61), 0.32 + 0.16 * sin(2.0 * u_scroll_progress + 1.2));
        color += vec3(0.0, 0.3, 0.2) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
