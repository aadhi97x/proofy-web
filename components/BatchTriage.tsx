        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <h2 className="text-6xl font-black tracking-tighter uppercase italic leading-[0.8] flex flex-col">
              ...
              <span className="text-neon">Signal Triage</span>
            </h2>
            <p className="text-white/30 text-lg italic font-light tracking-tight max-w-md">
              Parallel interrogation of multiple suspected neural fabrications.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="relative z-10 space-y-10">
            ...
            <div className="space-y-4">
              <h3 className="text-4xl font-black uppercase italic tracking-tighter text-white">Ingest Batch</h3>
              <p className="text-white/30 text-xl font-light italic max-w-sm mx-auto leading-relaxed">
                Stream multiple suspected neural assets for deep-layer verification.
              </p>
            </div>
            <div className="pt-6">
              <span className="text-[10px] font-black uppercase text-neon tracking-[0.6em] border border-neon/20 px-6 py-2 rounded-full">Secure Port Ready</span>
            </div>
          </div>
        </motion.div>
      ) : (
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between px-6">
              <span className="text-[11px] font-black uppercase text-white/20 tracking-[0.5em]">Triage Queue</span>
              <span className="text-[11px] font-mono text-white/40 uppercase tracking-widest">{files.length} ASSETS CONNECTED</span>
            </div>
                      ...
                      <div className="space-y-1.5 pt-1">
                        ...
                        <div className="flex items-center gap-4">
                          <span className={`text-[10px] font-mono font-bold uppercase tracking-widest ${res ? (res.verdict === 'REAL' ? 'text-emerald-500' : 'text-danger') : 'text-white/20'}`}>
                            {res ? `${res.verdict} // ${res.deepfakeProbability}% PROBABILITY` :
                              i === currentFileIndex && isProcessing ? 'Interrogating Layer_0x...' : 'Pending Extraction'}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-8 relative z-10">
                      {res && (
                        <div className="hidden md:flex flex-col text-right">
                          <span className="text-[10px] font-black text-white/20 uppercase tracking-widest mb-0.5">Confidence</span>
          ...
          <aside className="space-y-10">
            <div className="glass border border-white/5 p-12 rounded-[3rem] space-y-10 shadow-2xl sticky top-32">
              <div className="space-y-2">
                <span className="text-[11px] font-black uppercase text-neon tracking-[0.5em]">Batch Status</span>
                <h4 className="text-4xl font-black italic uppercase tracking-tighter underline decoration-neon/20 underline-offset-8">Neural Grid</h4>
              </div>
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    ...
                    <div className="text-right">
                      <span className="text-[10px] font-black text-white/20 uppercase tracking-widest block">Cycle Completion</span>
                      <span className="text-xs font-mono text-white/60">{results.length} / {files.length} SECURED</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                    <span className="text-[9px] font-black text-white/20 uppercase tracking-widest block mb-1">Anomalies</span>
                    <span className="text-2xl font-black text-danger">{results.filter(r => r.verdict === 'FAKE').length}</span>
                  </div>
                  <div className="p-6 bg-white/5 rounded-2xl border border-white/5">
                    <span className="text-[9px] font-black text-white/20 uppercase tracking-widest block mb-1">Verified</span>
                    <span className="text-2xl font-black text-emerald-500">{results.filter(r => r.verdict === 'REAL').length}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-white/5 flex flex-col gap-4">
                {!isProcessing ? (
                  ...
                  <div className="py-6 px-4 bg-neon/10 border border-neon/20 rounded-2xl flex items-center justify-center gap-4">
                    ...
                    <span className="text-[10px] font-black text-neon uppercase tracking-[0.4em]">Interrogating...</span>
                  </div>
                )}
              </div>
            </div>
