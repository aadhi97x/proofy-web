...
import { ArrowLeft, Search, ShieldCheck, AlertTriangle, ExternalLink, Ghost, Brain, FileSearch, Sparkles } from 'lucide-react';
      ...
      <header className="flex flex-col xl:flex-row xl:items-end justify-between gap-12">
        <div className="space-y-6">
          <button onClick={onBack} className="group flex items-center gap-4 text-white/20 hover:text-neon transition-all">
            ...
            <span className="text-[11px] font-black uppercase tracking-[0.4em]">Abort Session</span>
          </button>
          <div className="space-y-2">
            <h2 className="text-6xl font-black tracking-tighter uppercase italic leading-[0.8] flex flex-col">
              ...
              <span className="text-neon">Text Interrogator</span>
            </h2>
            <p className="text-white/30 text-lg italic font-light tracking-tight max-w-md">
              Decomposing semantic patterns to expose artificial construct signals and factual drift.
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="relative">
            <textarea
              ...
              disabled={isLoading}
              placeholder={mode === 'AI_DETECT' ? "Paste neural construct to analyze for artificial patterns..." : "Paste a claim or archival data to verify against global truth archives..."}
        ...
        <div className="lg:max-w-5xl mx-auto">
          {!isLoading ? (
            <motion.button
              ...
              disabled={!text.trim()}
              className="w-full py-8 bg-white text-charcoal font-black rounded-[2.5rem] hover:bg-neon transition-all shadow-[0_30px_60px_rgba(0,0,0,0.5)] disabled:opacity-10 uppercase tracking-[0.4em] text-xs flex items-center justify-center gap-4 group"
            >
              {mode === 'AI_DETECT' ? 'Interrogate Patterns' : 'Initiate Truth Search'}
              <Sparkles size={20} className="group-hover:rotate-12 transition-transform" />
            </motion.button>
          ) : (
            <div className="flex flex-col items-center py-12 animate-in fade-in duration-700">
              <NeuralLoader label={mode === 'AI_DETECT' ? 'Calibrating Neural Sifter' : 'Accessing Archive Nodes'} />
            </div>
          )}
        </div>
      </section>
            ...
            <div className="lg:col-span-4 space-y-8">
              <div className="p-12 glass border border-white/5 rounded-[3.5rem] space-y-12 shadow-2xl sticky top-32">
                <div className="text-center space-y-6">
                  <span className="text-[10px] text-white/20 font-black uppercase tracking-[0.5em] block">Neural Integrity Profile</span>
                ...
                {mode === 'AI_DETECT' && result.aiSignals && result.aiSignals.length > 0 && (
                  <div className="space-y-6 pt-10 border-t border-white/5">
                    <div className="space-y-4">
                      <p className="text-[11px] font-black text-white/20 uppercase tracking-[0.5em]">Anomalous Markers</p>
              ...
              <div className="p-16 glass border border-white/5 rounded-[4rem] space-y-12 shadow-2xl bg-surface/20">
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-[11px] font-black tracking-[0.6em] uppercase text-white/40">Forensic Briefing</h3>
                  </div>
                ...
                {result.claims && Array.isArray(result.claims) && result.claims.length > 0 && (
                  <div className="space-y-8 pt-12 border-t border-white/5">
                    <h4 className="text-[11px] font-black text-white/20 uppercase tracking-[0.6em]">Semantic Claim Decomposition</h4>
                ...
                {result.sources && Array.isArray(result.sources) && result.sources.length > 0 && (
                  <div className="space-y-8 pt-12 border-t border-white/5">
                    <h4 className="text-[11px] font-black text-white/20 uppercase tracking-[0.6em]">Authorized Archival Links</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {result.sources.map((s: any, i: number) => (
                        <a key={i} href={s.url} target="_blank" className="p-8 bg-charcoal/40 border border-white/5 rounded-[2.5rem] hover:bg-white/5 transition-all flex items-center justify-between group shadow-xl">
                          <div className="truncate pr-6 space-y-1">
                            <p className="text-lg font-black text-white/80 truncate group-hover:text-neon transition-colors italic uppercase tracking-tighter">{s.title}</p>
                            <p className="text-[10px] text-white/20 truncate font-mono opacity-50 lowercase">{s.url}</p>
                          </div>
                          <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center text-white/10 group-hover:text-neon group-hover:scale-110 transition-all">
